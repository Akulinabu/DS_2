...
# Каждый объект принадлежит к определенному типу
...
class Cat (object):
    ...
    def mur(self, name):
        self._name = name  #self. - при помощи селф (атрибут) можно использовать имя в разных методах, без него он используется только под своим методом
...
class B:
	...
	# не строго инкапсулированный метод
	def _method(self):
		print("Hello!")

	# "строго" инкапсулированный метод
	def __method_2(self):
		print("World!")
